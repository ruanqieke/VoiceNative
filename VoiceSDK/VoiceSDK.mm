...
static VoiceSDK* thisSDK = [[VoiceSDK alloc]init];
...
+(float)setVolume:(NSNumber *)volume{
    if(player){
        float tOldVolume = player.volume;
...
-(void) audioPlayerDidFinishPlaying:(AVAudioPlayer *)player successfully:(BOOL)flag{
    char tmp[255]= {0};
    sprintf(tmp, "cc.bk.anysdk.eval_OnPlayEnd()");
